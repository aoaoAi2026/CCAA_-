      ...
      <div v-else class="bg-white rounded-2xl shadow-xl p-8">
        ...
        <div v-if="!showResult" class="mb-6">
          ...
          <div v-if="showResultFeedback" class="mt-4 p-4 rounded-xl"
            ...
            <div class="font-semibold mb-1">{{ isCorrect ? '🎉 回答正确！' : '😅 答错了' }}</div>
            <div class="text-sm leading-relaxed">{{ currentQuestion.explanation }}</div>
        ...
        <div v-else class="text-center py-6">
          <div class="text-6xl mb-4">{{ correctCount / stageQuestions.length >= 0.7 ? '🏆' : '🌱' }}</div>
          <h2 class="text-3xl font-bold text-gray-800 mb-2">挑战完成！</h2>
          <p class="text-gray-600 mb-6">
            共 {{ stageQuestions.length }} 题，答对 {{ correctCount }} 题，得分 {{ score }} 分
          </p>
...
function startStage(chapterId) {
  const pool = questions['chapter' + chapterId] || []
  const qs = pool.length > 0 ? pool : fallbackPool
  stageQuestions.value = shuffle(qs.slice()).slice(0, Math.min(5, qs.length))
...
async function next() {
  ...
  if (auth.isLogin) {
    try {
      const add = correctCount.value * 5
      ...
      toast.show(`闯关完成！获得 ${add} 积分`, 'success')
    } catch (e) {}
    try {
      if (correctCount.value / stageQuestions.value.length >= 0.7) {
